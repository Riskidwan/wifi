...
Route::middleware('auth')->group(function () {
    ...
    // Route::get('/pelanggan/preview-kode', function () {
    //     $last = \App\Models\Pelanggan::orderBy('id_pelanggan', 'desc')->first();
    //     $nextNumber = $last ? (intval(substr($last->kode_pelanggan, 4)) + 1) : 1;
    //     $kode = 'PLG-' . str_pad($nextNumber, 3, '0', STR_PAD_LEFT);
    //     return response()->json(['kode' => $kode]);
    // })->name('pelanggan.preview.kode');
    ...
    // Route::get('/invoices/{invoice}/preview', [InvoiceController::class, 'previewPdf'])->name('invoices.pdf.preview');
...
Route::get('/payments/manual', [PaymentController::class, 'createManual'])->name('payments.create.manual'); // ✅ DI ATAS
...
Route::get('/api/pelanggan/{id}/unpaid-invoices', function ($id) {
    ...
    $invoices = $pelanggan->invoices->where('status', 'unpaid')->map(function ($inv) {
        ...
        // Jika paket tidak ditemukan, gunakan default
        if (!$paket) {
...
return [
    ...
    'ppn_value' => $ppn,        // ✅ tambahkan ini
    'diskon_persen' => $diskonPersen,
    'diskon_value' => $diskon,  // ✅ tambahkan ini
    'total' => $total,
];
    });
...
// ✅ Tambahkan di sini — DI DALAM grup auth
