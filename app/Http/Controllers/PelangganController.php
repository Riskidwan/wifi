...
$kodePelanggan = str_pad($nextNumber, 4, '0', STR_PAD_LEFT);
    ...
    //     $pppoeSuccess = $this->createPPPoESecret($pelanggan);

    // if (!$pppoeSuccess) {
    //     // Opsional: hapus pelanggan jika gagal buat PPPoE
    //     // $pelanggan->delete();
    //     return back()->withInput();
    // }
    // }
        // Buat di MikroTik
        $this->createPPPoESecret($pelanggan); // ✅ tanpa parameter
...
{
    $pelanggan = Pelanggan::with('paket')->findOrFail($id);
    return view('pelanggan.detail', compact('pelanggan')); // ✅ HARUS INI
}
...
// Tambahkan di PelangganController.php
